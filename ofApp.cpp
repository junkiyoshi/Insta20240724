...
void ofApp::update() {
	...
	for (int x = -1000; x <= 1000; x += size) {
		...
		for (int y = -1000; y <= 1000; y += size) {
			...
			auto height = ofMap(ofNoise(x * 0.0045, y * 0.0045, this->noise_step), 0, 1, 50, 2000);

			for (int z = 0; z <= height; z += size) {

				ofColor fill_color(ofMap(z, 0, 2000, 239, 0));
				this->setBoxToMesh(this->face, this->frame, glm::vec3(x, y, z), size, fill_color, ofColor(0));
			}
		}
